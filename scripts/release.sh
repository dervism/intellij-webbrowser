...
#   3. Refuse if that tag already exists (locally or on origin) — bump the
#      version in build.gradle.kts first.
#   4. If the working tree is dirty, offer to commit it, so the tag captures
#      exactly what ships.
#   5. ./gradlew clean check domainTest buildPlugin  (tests must pass first).
#   6. Push the branch, create an annotated tag vX.Y.Z, push the tag.
#   7. Create the GitHub release with the CHANGELOG's top section as the
#      notes, attaching the built plugin zip as a downloadable asset.
#
# Usage:
#   ./scripts/release.sh           # interactive (prompts before committing)
...
# ---- commit the working tree if needed -------------------------------------
if [ -n "$(git status --porcelain)" ]; then
  info "Working tree has uncommitted changes:"
  git status --short
  if confirm "Commit ALL of the above as \"Release $TAG\"?"; then
    git add -A
...
# ---- release notes = the top section of CHANGELOG.md -----------------------
NOTES_FILE="$(mktemp)"
